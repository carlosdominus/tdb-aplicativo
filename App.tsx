  ...
  return (
    <div className="min-h-screen pb-24 md:pb-0 bg-[#F8F9FA] overflow-x-hidden">
      ...
      <nav className="fixed bottom-0 left-0 right-0 glass-dark md:hidden z-50 flex items-center py-5 pl-6 pr-24 border-t border-white/5 justify-between shadow-2xl">
        <NavButton active={currentView === View.DASHBOARD} icon={<Home size={22} />} label="Home" onClick={() => navigateTo(View.DASHBOARD)} />
        <NavButton active={currentView === View.CATALOG} icon={<Beaker size={22} />} label="Catálogo" onClick={() => navigateTo(View.CATALOG)} />
        <NavButton active={currentView === View.TRACKER} icon={<Zap size={22} />} label="Turbo" onClick={() => navigateTo(View.TRACKER)} />
        <NavButton active={currentView === View.BONUSES} icon={<Gift size={22} />} label="Bônus" onClick={() => navigateTo(View.BONUSES)} />
      </nav>
    </div>
  );
};
